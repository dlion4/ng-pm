...
export interface NotificationItem {
  id: number; icon: string; tone: string; title: string; desc: string; time: string; unread: boolean;
}

export interface ToastItem {
  id: number; message: string; type: 'success' | 'danger' | 'warning' | 'info';
  isLeaving: boolean; title: string; icon: string; bg: string; color: string;
}

@Component({
  selector: 'app-dashboard-dev-header',
  standalone: true,
  imports: [CommonModule, RouterModule],
  templateUrl: './dashboard-dev-header.html',
  styleUrls: ['./dashboard-dev-header.css'],
  encapsulation: ViewEncapsulation.None
})
export class DashboardDevHeaderComponent implements OnInit {
  ...
  @HostListener('document:click', ['$event']) onDocumentClick(e: MouseEvent): void {
    if (!(e.target as HTMLElement).closest('[data-dropdown]')) this.closeAllDropdowns();
  }
  @HostListener('document:keydown', ['$event']) onKeydown(e: KeyboardEvent): void {
    if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'b') { e.preventDefault(); this.toggleSidebar.emit(); }
  ...
  toggleDropdown(name: string, e?: MouseEvent): void {
    ...
    if (this.openDropdown === name) { this.closeAllDropdowns(); return; }
    this.closeAllDropdowns(); this.openDropdown = name;
  }
  ...
  onOpenAside(type: string, e?: MouseEvent): void {
    if (e) e.stopPropagation(); this.openAside.emit(type); this.closeAllDropdowns();
  }
  ...
  addToast(message: string, type: 'success' | 'danger' | 'warning' | 'info'): void {
    ...
    this.toasts.push(toast); this.toast.emit(toast);
    setTimeout(() => this.removeToast(toast.id), 4500);
  }

  removeToast(id: number): void {
    const idx = this.toasts.findIndex(t => t.id === id);
    if (idx > -1) { this.toasts[idx] = { ...this.toasts[idx], isLeaving: true }; setTimeout(() => { this.toasts = this.toasts.filter(t => t.id !== id); }, 300); }
  }
}
