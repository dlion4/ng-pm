#!/usr/bin/env python3
"""
Angular 17+ Control Flow FINAL FIXER v5
Aggressively destroys all stray }, fixes inline @if, nukes orphaned @empty.
"""
...
def process(filepath):
    ...
    for line in lines:
        ...
        # This matches "}", "  }", "    }", etc. even with trailing spaces
        if re.match(r'^\s*\}\s*$', s):
            if stack:
                stack.pop()
                out.append(line)
...
def main():
    ...
    base = Path(BASE_PATH)
    print("=" * 60)
    print("  ANGULAR CONTROL FLOW FIXER v5")
